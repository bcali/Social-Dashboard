...
import { ArrowDown, ArrowUp, ArrowUpDown, Search, Maximize2 } from "lucide-react";
import { Badge, Input, Overlay } from "@/components/ui";
...
export function PropertyRankingTable({ hotels, baseline }: PropertyRankingTableProps) {
  ...
  const tableContent = (maxHeight: string) => (
    <div className={`${maxHeight} overflow-y-auto overflow-x-auto`}>
      <table className="ui-table w-full">
        ...
        <tbody>
          {baseline && (
            <tr className="ui-glow-success [&>td]:sticky [&>td]:top-[33px] [&>td]:z-[9] [&>td]:bg-[var(--bg-card)]">
  ...
  return (
    <>
      <div className="ui-card overflow-hidden">
        ...
        {tableContent("max-h-[480px]")}
      </div>

      <Overlay open={expanded} onClose={() => setExpanded(false)} title="Property Rankings" maxWidth="full">
